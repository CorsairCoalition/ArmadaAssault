...
export default class Algorithms {
	...
	static bfs(gameMap: GameMap, node: number, radius: number) {
		...
		while(queue.length > 0) {
			...
			let adjacentTiles = gameMap.getAdjacentTiles(curTile)
			...
			for(let direction in adjacentTiles) {
				if (adjacentTiles.hasOwnProperty(direction)) {
					...
					if(!isVisited[nextTile.index]) {
						...
						if(gameMap.isWalkable(nextTile)) {
	...
	static aStar(gameMap, start:number, ends: number[]) {
		return AStar.search(gameMap, start, ends)
	}

	//returns shortest path (as array) between start and end index
	//no node weights
	static dijkstra(gameMap, start, end) {
		let isVisited = []
		let previous = []
	...
	static decisionTreeSearch(gameMap, startPoints, turns) {
	...
	static decisionTreeSearchRec(start: number, turns: number, weight = 0) {
		let possibleMoves = []

		if(turns != 0) {
			let adjacentTiles = this.gameMap.getAdjacentTiles(start)
			//loop through adjacent tiles
			for(let direction in adjacentTiles) {
				if (adjacentTiles.hasOwnProperty(direction)) {
					let nextTile = adjacentTiles[direction]
					if(this.gameMap.isWalkable(nextTile)) {
