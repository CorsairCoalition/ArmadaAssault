...
export default class AStar {
	static TILE_COST: number

	static init(gameMap: GameMap) {
		AStar.TILE_COST = 1
		let searchList = []
	...
	static search(gameMap: GameMap, start: number, ends: number[]) {
		let searchList = this.init(gameMap)
		...
		while(openHeap.size() > 0) {
			...
			let adjacentTiles = gameMap.getAdjacentTiles(curTile.index)
			for(let direction in adjacentTiles) {
				if (adjacentTiles.hasOwnProperty(direction)) {
					...
					if(nextTile.value >= 0 && nextTile.value != gameMap.playerIndex) {
						gScore += gameMap.armies[nextTile.index]
					...
					if(!visited || gScore < neighbour.g) {
						...
						neighbour.armies = gameMap.armies[nextTile.index]
